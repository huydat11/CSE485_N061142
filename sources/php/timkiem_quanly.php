...
<head>
  ...
  $(".xoa").click(function(){
    ...
    $('.timkiem').keyup(function(){//bắt sự kiện khi nhả phím
     //alert($('.timkiem').val());
     var txt=$('.timkiem').val();
     //alert(txt);
     $.post('ajax.php',{data: txt},function(data){//đổ dữ liệu sang file ajax.php
     $.('.danhsach').html(data);  //sau khi xử lý file ajax.php thì dữ liệu sẽ được đổ vào class danhsach
      })
    })
  })
</script>
